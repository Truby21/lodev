...
/*   Updated: 2021/03/21 22:11:45 by truby            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_texture(t_data *data)
{
 // double posX = 22, posY = 12;  //x and y start position   							//положение игрока
  double dirX = 0, dirY = -1; //initial direction vector							//направление взгляда
...
//   double oldTime = 0; //time of previous frame
  while(1)
  {
    while (x++ < w)
    {
      ...
      int mapX = (int)data->param.plx;														//константа с положением игрока по х
      int mapY = (int)data->param.ply;														//константа с положением игрока по у
      ...
      while (hit == 0)
      {
        ...
        if(data->param.map[mapX][mapY] > 0) hit = 1;
      }
      //Calculate distance projected on camera direction (Euclidean distance will give fisheye effect!)
      if(side == 0) perpWallDist = (mapX - data->param.plx + (1 - stepX) / 2) / rayDirX;		//расстояние до стены
      else          perpWallDist = (mapY - data->param.ply + (1 - stepY) / 2) / rayDirY;
    ...
    //   }
	  ...
	  while (++y < drawEnd)
	  {
		my_mlx_pixel_put(data, x, y, 0xFF0000);										//рисуем стены
	  }
    }
