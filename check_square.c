...
/*   Updated: 2021/03/16 15:56:50 by truby            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_param			*ft_check_right_line(t_param *param, int end, int ml)
{
	...
	while (++i < end)
	{
		if (ft_isprint(param->map[i][ml]) && param->map[i][ml] !=
		'1' && param->map[i][ml] != ' ')
			return (ft_error("Error\nInvalid map.\n"));
	}
	return (param);
...
{
	if (!ft_check_first_line(param, -1))
		return (NULL);
	if (!ft_check_last_line(param, end, -1))
		return (NULL);
	if (!ft_check_left_line(param, end, -1))
		return (NULL);
	if (!ft_check_right_line(param, end, ml))
		return (NULL);
	return (param);
}
